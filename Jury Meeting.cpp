...
void solve() {
	...
	while (t--) {
		...
		if ((vals.back()).ss >= 2 || sz(vals) == 1) {
			cout << fact[n] << nline;
		} else {
			pair<int, int> p1 = vals[sz(vals) - 1];
			pair<int, int> p2 = vals[sz(vals) - 2];
			if (p2.ff < p1.ff - 1) {
				cout << 0 << nline;
			} else {
				reverse(all(vals));
				ll places = p2.ss + 1;
				ll ways = combination(n, places, MOD1, fact, ifact);
				ways = mod_mul(ways, fact[p2.ss], MOD1);
				ways = mod_mul(ways, fact[n - p2.ss - 1], MOD1);
				ll ways4 = fact[n];
				debug(ways)
				debug(ways4)
				ll ans = mod_sub(ways4, ways, MOD1);
				cout << ans << nline;
			}
		}
	}
